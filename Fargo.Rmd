...
Before applying time series forecasting models, we will fit several simple trend models to examine the overall pattern in the data.
...
# Fit trend models
fit_linear <- lm(IncomingExaminations ~ t, data = data)
fit_quad   <- lm(IncomingExaminations ~ t + I(t^2), data = data)
fit_cube   <- lm(IncomingExaminations ~ t + I(t^2) + I(t^3), data = data)
fit_exp    <- lm(log(IncomingExaminations) ~ t, data = data)
...
# Visualize models
plot(data$t, data$IncomingExaminations); lines(fitted(fit_linear), col="blue")
plot(data$t, data$IncomingExaminations); lines(fitted(fit_quad), col="red")
plot(data$t, data$IncomingExaminations); lines(fitted(fit_cube), col="green")
plot(data$t, data$IncomingExaminations); lines(exp(fitted(fit_exp)), col="purple")
```
...
fit_stl <- stl(ts_data, 
               s.window = "periodic") 
# seasonal component estimated as periodic function.
# "periodic" assumes the seasonal pattern repeats identically each year.


# Visualize STL decomposition
plot(fit_stl,
     main = "STL Decomposition of Incoming Examinations")
